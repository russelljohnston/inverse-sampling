...
for i in range(Nsamp):
    ...
    axs[0].hist(ss,sbins,histtype='step',color='r',alpha=0.1,label='MC samples')
...
R=(MCmean - ndatabin)/np.sqrt(VarMC)
#remove inf caused by 0/0
R[np.isnan(R)] = 0.
R[R<-10e10] = 0.

#plot data - black line
axs[0].hist(data,sbins,histtype='step',color='k')

#plot residuals
mcbinloc = mcbinloc[0:len(sbins)-1]
